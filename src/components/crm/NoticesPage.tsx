...
const ALL_ROLES = ['admin', 'director'];

const emptyForm = {
  title: '',
  content: '',
  category: 'announcement',
  targetRoles: ['admin', 'director'] as string[],
};

export function NoticesPage() {
  const { user, setCurrentPage, unreadNoticeCount, setUnreadNoticeCount } = useAppStore();
  ...
  const fetchNotices = () => {
    fetchWithAuth('/api/notices').then((d) => {
      setNotices(d);
    });
  };

  const fetchUpcomingEvents = () => {
    fetchWithAuth('/api/events?status=upcoming&limit=20').then((res) => {
  ...
  // Mark all notifications as read when visiting notices page
  useEffect(() => {
    if (user && unreadNoticeCount > 0) {
      fetchWithAuth('/api/notifications/mark-all-read', { method: 'PUT' })
        .then(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setForm({ ...emptyForm });
      fetchNotices();
      // Refresh unread count so the red dot updates for other users
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetchWithAuth(`/api/notices?id=${id}`, {
        method: 'DELETE',
      });
      fetchNotices();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
          ...
          <Dialog open={open} onOpenChange={(v) => { if (!v) { setEditId(null); setForm({ ...emptyForm }); } setOpen(v); }}>
            ...
            <DialogContent className="max-w-lg">
              <DialogHeader>
                ...
                <p className="text-sm text-muted-foreground">{editId ? '修改公告內容及分類' : '發佈新公告通知所有成員'}</p>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={() => { setOpen(false); setEditId(null); setForm({ ...emptyForm }); }}
                  >
                    取消
                  </Button>
                  <Button
              ...
              .map((event) => {
                ...
                return (
                  <Card
                    key={event.id}
                    className={cn(
                      'transition-all hover:shadow-sm border-l-4 border-l-red-400 bg-gradient-to-r from-red-50/50 to-transparent'
                    )}
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex gap-3 flex-1 min-w-0">
                          ...
                          {event.imageUrl && (
                            <div
                              className="shrink-0 w-20 h-20 rounded-md overflow-hidden cursor-pointer bg-muted"
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-2 flex items-center gap-2 flex-wrap">
                              <Badge
                                variant="outline"
                                className={cn('text-[10px] border', EVENT_TYPE_COLORS[event.type])}
                              >
                            ...
                            <div className="mt-2 flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span>
                                  {eventDate.toLocaleString('zh-HK', {
                                    month: 'short',
                                    day: 'numeric',
                                    weekday: 'short',
                                    hour: '2-digit',
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <span>{event._count.participants}/{event.maxAttendees} 人</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Expanded section with full poster + description */}
      ...
      <div className="space-y-3 max-h-[calc(100vh-250px)] overflow-y-auto pr-1">
        {/* Section header for notices */}
        {notices.length > 0 && (
          <div className="flex items-center gap-2 sticky top-0 bg-background py-1 z-10">
            <Megaphone className="h-4 w-4 text-amber-600" />
            <span className="text-sm font-medium text-muted-foreground">系統公告</span>
          </div>
        )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                      ...
                      {[...new Set(notice.targetRoles.split(','))].map((role) => (
                        <Badge
                          key={role}
                          variant="outline"
        ...
        {notices.length === 0 && upcomingEvents.length === 0 && (
