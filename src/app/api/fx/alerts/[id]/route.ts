...
export async function PATCH(
  ...
  try {
    ...
    const existing = await db.currencyAlert.findUnique({ where: { id } });
    if (!existing) return NextResponse.json({ error: '找不到預警' }, { status: 404 });
    ...
    const updated = await db.currencyAlert.update({
      where: { id },
      data: updateData,
    });
...
export async function DELETE(
  ...
  try {
    ...
    await db.currencyAlert.delete({ where: { id } });
