 ...
 * 11 parent products + all sub-products (matching screenshot data)
...
async function main() {
  ...
  await prisma.user.create({ data: { email: 'chan@mypath.hk', name: '陳志明', password: pwd, role: 'director', phone: '+85291234567' } });
  await prisma.user.create({ data: { email: 'lee@mypath.hk', name: '李美琪', password: pwd, role: 'director' } });
  await prisma.user.create({ data: { email: 'wong@mypath.hk', name: '黃偉業', password: pwd, role: 'director' } });
  await prisma.user.create({ data: { email: 'bosco@mclub.com', name: 'Bosco', password: pwd, role: 'director' } });
  await prisma.user.create({ data: { email: 'damon@mclub.com', name: 'Damon Lewis', password: pwd, role: 'director' } });
  ...
  for (const p of parents) { await prisma.product.create({ data: p }); console.log(`  Parent: ${p.name}`); }
  ...
  allSubs.push(
    ...
    sp('lg-07','pp-009','臨時命令申請（禁制令/居住令）','Interim Order Application (Injunction/Occupation)','⚖️','legal-hk-divorce',10000,18000,0,0),
  );

  // === 中港離婚收費表 (7) ===
  allSubs.push(
    sp('lg-08','pp-009','中港跨境離婚（香港訴訟）','Cross-border Divorce (HK Litigation)','⚖️','legal-cn-divorce',50000,80000,0,0),
    sp('lg-09','pp-009','中港跨境離婚（內地訴訟）','Cross-border Divorce (Mainland Litigation)','⚖️','legal-cn-divorce',40000,70000,0,0),
    ...
    sp('lg-12','pp-009','中港子女撫養權協調','Cross-border Child Custody Coordination','⚖️','legal-cn-divorce',25000,50000,0,0),
    sp('lg-13','pp-009','跨境財產追索及分割','Cross-border Asset Recovery & Division','⚖️','legal-cn-divorce',40000,100000,0,0),
    sp('lg-14','pp-009','中港離婚公證及文件認證','Cross-border Divorce Notarization','⚖️','legal-cn-divorce',5000,12000,0,0),
  );

  // === 中港離婚附加選項 (9) ===
  allSubs.push(
    sp('lg-15','pp-009','CN 律師推介及配合','CN Lawyer Referral & Coordination','⚖️','legal-cn-options',10000,20000,0,0),
    sp('lg-16','pp-009','跨境資產調查 (內地)','Cross-border Asset Investigation (Mainland)','⚖️','legal-cn-options',20000,60000,0,0),
    sp('lg-17','pp-009','強制執行 (跨境)','Enforcement (Cross-border)','⚖️','legal-cn-options',30000,80000,0,0),
    sp('lg-18','pp-009','婚姻資產保護規劃','Matrimonial Asset Protection Planning','⚖️','legal-cn-options',15000,40000,0,0),
    sp('lg-19','pp-009','法庭聆訊代表 (每次)','Court Hearing Representation (Per Session)','⚖️','legal-cn-options',5000,10000,0,0),
    ...
    sp('lg-23','pp-009','外地法庭代表 (不含香港)','Overseas Court Representation (Excl. HK)','⚖️','legal-cn-options',15000,40000,0,0),
  );

  // === 離婚服務額外收費 (5) ===
  allSubs.push(
    sp('lg-24','pp-009','遺囑撰寫及見證','Will Drafting & Witnessing','⚖️','legal-extra',5000,12000,0,0),
    sp('lg-25','pp-009','遺產承辦及管理','Estate Administration & Probate','⚖️','legal-extra',30000,80000,0,0),
    sp('lg-26','pp-009','遺產爭訟','Estate Litigation','⚖️','legal-extra',50000,150000,0,0),
    sp('lg-27','pp-009','持久授權書','Enduring Power of Attorney (EPA)','⚖️','legal-extra',8000,15000,0,0),
  ...
  allSubs.push(
    ...
    sp('lg-31','pp-009','商業糾紛調解及仲裁','Commercial Dispute Mediation & Arbitration','⚖️','legal-represent',30000,150000,0,0),
    ...
    sp('lg-35','pp-009','勞資糾紛及僱傭問題','Employment Disputes & Labour Issues','⚖️','legal-represent',15000,80000,0,0),
    sp('lg-36','pp-009','土地糾紛及業權爭議','Land Disputes & Title Disputes','⚖️','legal-represent',40000,200000,0,0),
    sp('lg-37','pp-009','移民法律諮詢及上訴','Immigration Legal Advisory & Appeals','⚖️','legal-represent',15000,60000,0,0),
  );

  // === 刑事案件 (4) ===
  allSubs.push(
    sp('lg-38','pp-009','刑事辯護 - 裁判法院','Criminal Defence - Magistrates Court','⚖️','legal-criminal',20000,80000,0,0),
    ...
    sp('lg-41','pp-009','刑事法律諮詢及保釋','Criminal Legal Consultation & Bail','⚖️','legal-criminal',5000,20000,0,0),
  );
  ...
  // 樂天黃金CFD (1)
  allSubs.push(
    sp('gold-1','pp-011','Standard Gold CFD Trading Account','Standard Gold CFD Trading Account','🥇','investment',2000,100000,0,0,true),
  );

  // Insert all sub-products
  for (const s of allSubs) { await prisma.product.create({ data: s }); }
  console.log(`Created ${allSubs.length} sub-products\n`);

  // Events
  const e1 = await prisma.event.create({ data: { title: 'MCLUB 2026 夏季投資論壇', description: '探討家族辦公室資產配置策略', type: 'seminar', date: new Date('2026-07-15T14:00:00'), location: '香港四季酒店宴會廳', maxAttendees: 50, status: 'upcoming' } });
  const e2 = await prisma.event.create({ data: { title: '日本物業投資說明會', description: '大阪物業投資機會深度解析', type: 'webinar', date: new Date('2026-07-22T10:00:00'), location: '線上 Zoom 會議', maxAttendees: 30, status: 'upcoming' } });
  await prisma.eventParticipant.createMany({ data: [
    { eventId: e1.id, userId: admin.id, status: 'registered' },
    { eventId: e2.id, userId: admin.id, status: 'registered' },
  ]});

  // Notices
  await prisma.notice.createMany({ data: [
    { title: '系統升級通知', content: 'MCLUB CRM已升級至v3.0版本。', category: 'announcement', targetRoles: 'admin,director', authorId: admin.id, isPinned: true, isActive: true },
    { title: '夏季投資論壇報名開放', content: 'MCLUB 2026夏季投資論壇現已接受報名。', category: 'announcement', targetRoles: 'admin,director', authorId: admin.id, isPinned: false, isActive: true },
  ]});
  ...
  console.log(`  Total Prods:   ${totalParents + totalChildren}`);
