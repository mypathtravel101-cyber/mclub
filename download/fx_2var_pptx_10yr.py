...
tb(sl,Inches(1),Inches(2.9),Inches(11),Inches(1.5),
   '\u65e5\u672c\u7269\u696d\u6295\u8cc7 \u2014 \u96d9\u8b8a\u91cf\u58d3\u529b\u6e2c\u8a66',sz=34,b=True,c=WHITE,a=PP_ALIGN.CENTER)
tb(sl,Inches(1),Inches(4.2),Inches(11),Inches(0.5),
   '2-Variable Stress Test: FX Rate x Property Value ({}-Year Holding)'.format(HOLD),sz=16,c=GREY,a=PP_ALIGN.CENTER)
...
cv = (
    ...
    "\u5e74\u79df\u91d1: JPY {:,.0f} (6%)\n"
    "\u5ba2\u6236\u6295\u5165: HKD {:,.0f}".format(
        HOLD, C['monthly_payment_jpy'], C['annual_rent_jpy'], C['invested_hkd'])
)
vr = (
    "VARIABLE \u8b8a\u91cf\u6578\u64da\n\n"
...
tb(sl,Inches(0.6),Inches(0.2),Inches(12),Inches(0.7),
   '\u96d9\u8b8a\u91cf\u58d3\u529b\u6e2c\u8a66 \u2014 84\u500b\u60c5\u666f\u5e74\u5316\u56de\u5831\u77e9\u9663 ({} \u5e74)'.format(HOLD),
...
sl=prs.slides.add_slide(prs.slide_layouts[6]); bg(sl)
tb(sl,Inches(0.6),Inches(0.2),Inches(12),Inches(0.7),
   '\u4e0d\u540c\u623f\u50f9\u60c5\u666f\u4e0b\u7684\u532f\u7387\u654f\u611f\u5ea6\u66f2\u7dda ({} \u5e74)'.format(HOLD),
   sz=24,b=True,c=WHITE)
bar(sl,Inches(0.85))
sl.shapes.add_picture('{}/stress_2var_lines.png'.format(CH),Inches(0.3),Inches(1.0),Inches(12.7),Inches(5.6))
ft(sl)

# SLIDE 4: CONTOUR / RISK MAP
sl=prs.slides.add_slide(prs.slide_layouts[6]); bg(sl)
tb(sl,Inches(0.6),Inches(0.2),Inches(12),Inches(0.7),
   '\u76c8\u8667\u5e73\u8861\u98a8\u96aa\u5730\u5716 \u2014 FX Rate x Property Value ({} \u5e74)'.format(HOLD),
   sz=24,b=True,c=WHITE)
bar(sl,Inches(0.85))
sl.shapes.add_picture('{}/stress_2var_contour.png'.format(CH),Inches(0.3),Inches(1.0),Inches(12.7),Inches(5.6))
ft(sl)

# SLIDE 5: FULL RESULTS TABLE
sl=prs.slides.add_slide(prs.slide_layouts[6]); bg(sl)
tb(sl,Inches(0.6),Inches(0.2),Inches(12),Inches(0.7),
   '\u58d3\u529b\u6e2c\u8a66\u5b8c\u6574\u7d50\u679c\u8868 ({} \u5e74\u5e74\u5316\u56de\u5831)'.format(HOLD),
   sz=24,b=True,c=WHITE)
bar(sl,Inches(0.85))
...
for ri, (pl, row_vals) in enumerate(zip(prop_labels, mat)):
    ...
    for ci, v in enumerate(row_vals):
        cell=tbl.cell(ri+1,ci+1); cell.text='{:.2f}%'.format(v)
        for p in cell.text_frame.paragraphs:
            p.font.size=Pt(8.5); p.font.name='Sarasa Mono SC'; p.alignment=PP_ALIGN.CENTER
            p.font.bold = (ci == 5 and ri == 2)
            if v >= 10: p.font.color.rgb = GREEN
            elif v >= 7: p.font.color.rgb = RGBColor(0x82,0xE0,0xAA)
...
findings = [
    ...
    ("\u69d3\u7aef\u5b89\u5168\u908a\u969b",
     ...
     "\u6309\u63d0\u672c\u91d1\u511f\u9084\u6301\u7e8c\u964d\u4f4e\u8cb8\u6b3e\u9918\u984d\uff0c{} \u5e74\u5167\u5df2\u511f\u908461.6%\u672c\u91d1".format(HOLD),
     RGBColor(0x34,0x98,0xDB)),
    ("\u96d9\u91cd\u98a8\u96aa\u6700\u5dee\u60c5\u666f",
     "\u623f\u50f9-30% + JPY\u8cb631%\uff082022\u5371\u6a5f\uff09\n"
     "\u5e74\u5316{:.2f}%\uff0c{} \u5e74\u7e3dROI +3.6%\uff0c\u4ecd\u80fd\u4fdd\u672c\u5e76\u7372\u5f97\u6b63\u56de\u5831".format(D['worst']['annualized'], HOLD),
     RED),
]
...
tb(sl,Inches(8.5),Inches(1.25),Inches(4.1),Inches(0.4),
   '\u98a8\u96aa\u8a55\u7d1a\u7e3d\u7d50 ({} \u5e74)'.format(HOLD),sz=15,b=True,c=GOLD,a=PP_ALIGN.CENTER)

grades = [
    ("\u6574\u9ad4\u98a8\u96aa\u7b49\u7d1a", "LOW-MODERATE\n\u4e2d\u7b49\u504f\u4f4e", GREEN),
    ("FX\u98a8\u96aa", "MITIGATED\n\u5df2\u88ab\u81ea\u7136\u5c0d\u6c96\u7de9\u89e3", RGBColor(0x82,0xE0,0xAA)),
    ("\u623f\u50f9\u98a8\u96aa", "LOW-MODERATE\n{} \u5e74\u6301\u6709\u5206\u6563\u98a8\u96aa".format(HOLD), GOLD),
    ("\u5229\u7387\u98a8\u96aa", "LOW\n\u65e5\u672c3%\u8655\u6b77\u53f2\u4f4e\u4f4d", GREEN),
    ("\u6d41\u52d5\u6027\u98a8\u96aa", "MODERATE\n\u65e5\u672c\u7269\u696d\u6d41\u52d5\u6027\u504f\u4f4e", ORANGE),
    ("\u5efa\u8b70", "\u9069\u7528MCLUB\u9032\u968e\u7248\n(Advanced)\u670d\u52d9\u5c64\u7d1a\nHK$128,000", PURPLE),
]
