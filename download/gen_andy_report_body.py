...
def build_body():
    ...
    elements.append(Paragraph(
        '本報告採用 84 情景壓力測試結合機器學習機率預測模型，對客戶 Andy 以港幣 320 萬（物業總價）'
        '透過銀行按揭（40% LTV）投資日本物業之回報進行全面評估。'
    ...
    elements.append(Paragraph(
        ...
        '折合日圓 62,400,000 元（約 6,240 萬日圓）。Andy 向銀行申請按揭貸款，'
        '按揭成數為 40% LTV（借入約 HKD 128 萬），自付 60% 首期即約 HKD 192 萬。'
        '入場時之匯率為 19.5 日圓兌 1 港元（JPY/HKD）。'
    ...
    elements.append(Paragraph(
        ...
        '第三，即使經 ML 調整後，10 年持有期之預期回報仍高達 +84.0%，代表 Andy 之 HKD 192 萬首期本金'
        '預期可增值至約 HKD 353 萬（192 萬 x 1.84 = 353 萬），名義利潤約 HKD 161 萬。'
    ...
    elements.append(colored_box(
        ...
        'Andy 透過銀行按揭購入大阪物業（總價 HKD 320 萬，自付首期 HKD 192 萬，40% LTV），'
        '在 10 年持有期下：<br/>'
        '- <b>最可能情景</b>：獲利約 +84%（首期本金增值至約 HKD 353 萬）<br/>'
        '- <b>保守估計（P5）</b>：獲利至少 +54%（首期本金增值至約 HKD 296 萬）<br/>'
        '- <b>機率結論</b>：超過 95% 機率獲得正回報<br/><br/>'
        '<b>結論：Andy 以銀行按揭方式投資，在 ML 模型涵蓋之所有情景中，10 年持有期之投資回報極大概率為正。</b>',
