...
def build_body():
    ...
    findings = [
        '<b>回報前景樂觀：</b>Andy 透過銀行按揭購買日本大阪物業（40% LTV，年利率 3%），'
        '自付 HKD 320 萬作為首期本金。ML 機率加權模型預測 10 年持有期之預期回報為 +84.0%，'
        '即首期 HKD 320 萬本金預期增值至約 HKD 588 萬，名義利潤約 HKD 268 萬。'
    ...
    elements.append(Paragraph(
        '基於三層分析架構之綜合評估，Andy 透過銀行按揭（自付 60% 首期即 HKD 320 萬，'
        '向銀行借入 40% 按揭貸款，年利率 3%）購入日本大阪物業之方案，在歷史數據支持、'
    ...
    elements.append(colored_box(
        ...
        'Andy 透過銀行按揭（40% LTV）購入大阪物業，自付 HKD 320 萬首期，在 10 年持有期下：<br/>'
        '- <b>最可能情景</b>：獲利約 +84%（首期本金增值至約 HKD 588 萬）<br/>'
        '- <b>保守估計（P5）</b>：獲利至少 +54%（首期本金增值至約 HKD 493 萬）<br/>'
